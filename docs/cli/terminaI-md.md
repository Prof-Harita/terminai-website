...
1.  **Global context file:**
    - **Location:** `~/.terminai/terminaI.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `terminaI.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `terminaI.md` files in subdirectories
...
### Example `terminaI.md` file

Here is an example of what you can include in a `terminaI.md` file at the root
...
- **`/memory refresh`**: Forces a re-scan and reload of all `terminaI.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.terminai/terminaI.md` file. This lets you add persistent memories on the
  fly.

## Modularize context with imports

You can break down large `terminaI.md` files into smaller, more manageable
...
**Example `terminaI.md` with imports:**

```markdown
# Main terminaI.md file
...
While `terminaI.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "terminaI.md"]
  }
}
```
