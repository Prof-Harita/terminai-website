...
The Gemini CLI includes built-in tools that the LLM uses to interact
...
the LLM can request to be executed. For example, if you ask Gemini to
...
definitions (schemas) to the LLM, executes them when requested, and
...
3.  The core, along with your prompt and conversation history, sends a list of
    available tools and their descriptions/schemas to the LLM API.
4.  The LLM analyzes your request. If it determines that a tool is
...
6.  The output from the tool is sent back to the LLM.
7.  The LLM uses the tool's output to formulate its final answer, which
...
- **`process_manager`:** Named long-running sessions with bounded output and
  safe termination. See
  [TerminaI process manager](../termai-process-manager.md).
