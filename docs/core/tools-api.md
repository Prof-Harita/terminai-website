...
LLM, allowing it to interact with the local environment, fetch web
...
- **Tool (`tools.ts`):** An interface and base class (`BaseTool`) that defines
  ...
  - `parameterSchema`: A JSON schema defining the parameters that the tool
    accepts. This is crucial for the LLM to understand how to call the
    tool correctly.
...
- **Tool registry (`tool-registry.ts`):** A class (`ToolRegistry`) responsible
  ...
  - **Providing schemas:** Exposing the `FunctionDeclaration` schemas of all
    registered tools to the LLM, so it knows what tools are available
    and how to use them.
...
1.  **Model request:** The LLM, based on the user's prompt and the
...
8.  **Response to model:** The `llmContent` from the `ToolResult` is packaged as
    a `FunctionResponse` and sent back to the LLM so it can continue
