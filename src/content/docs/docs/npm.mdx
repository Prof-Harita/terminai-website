...
This monorepo contains two main packages: `@google/terminai` and `@google/terminai-core`.

## `@google/terminai`

[Section titled “@google/terminai”](#googleterminai)

This is the main package for the TerminaI. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package’s dependencies, including `@google/terminai-core`. This means that whether a user installs the package with `npm install -g @google/terminai` or runs it directly with `npx @google/terminai`, they are using this single, self-contained executable.

## `@google/terminai-core`

[Section titled “@google/terminai-core”](#googleterminai-core)

This package contains the core logic for interacting with the LLM API. It is responsible for making API requests, handling authentication, and managing the local cache.
...
*   **Simplified script execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @google/terminai`.