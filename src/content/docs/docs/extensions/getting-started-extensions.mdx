---
title: "Getting started with TerminaI extensions"
description: ""
---

# Getting started with TerminaI extensions


This guide will walk you through creating your first TerminaI extension. You’ll learn how to set up a new extension, add a custom tool via an MCP server, create a custom command, and provide context to the model with a `GEMINI.md` file.

## Prerequisites

[Section titled “Prerequisites”](#prerequisites)

Before you start, make sure you have the TerminaI installed and a basic understanding of Node.js and TypeScript.
...
```
my-first-extension/├── example.ts├── gemini-extension.json├── package.json└── tsconfig.json
```
...
This is the manifest file for your extension. It tells TerminaI how to load and use your extension.
...
*   `mcpServers`: This section defines one or more Model Context Protocol (MCP) servers. MCP servers are how you can add new tools for the model to use.
    *   `command`, `args`, `cwd`: These fields specify how to start your server. Notice the use of the `${extensionPath}` variable, which TerminaI replaces with the absolute path to your extension’s installation directory. This allows your extension to work regardless of where it’s installed.
...
Before you can use the extension, you need to compile the TypeScript code and link the extension to your TerminaI installation for local development.
...
3.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the TerminaI extensions directory to your development directory. This means any changes you make will be reflected immediately without needing to reinstall.
...
Now, restart your TerminaI session. The new `fetch_posts` tool will be available. You can test it by asking: “fetch posts”.
...
After saving the file, restart the TerminaI. You can now run `/fs:grep-code "some pattern"` to use your new command.
...
2.  Update your `gemini-extension.json` to tell the CLI to load this file:
    ...
    ```
    {  "name": "my-first-extension",  "version": "1.0.0",  "contextFileName": "GEMINI.md",  "mcpServers": {    "nodeServer": {      "command": "node",      "args": ["${extensionPath}${/}dist${/}example.js"],      "cwd": "${extensionPath}"    }  }}
    ```
...
For detailed instructions on both methods, please refer to the [Extension Releasing Guide](/docs/extensions/extension-releasing).

## Conclusion

[Section titled “Conclusion”](#conclusion)

You’ve successfully created a TerminaI extension! You learned how to:
...
From here, you can explore more advanced features and build powerful new capabilities into the TerminaI.