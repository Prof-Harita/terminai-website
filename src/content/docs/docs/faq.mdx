...
This page provides answers to common questions and solutions to frequent problems encountered while using TerminaI.
...
This error indicates that you have exceeded your API request limit. The LLM API has rate limits to prevent abuse and ensure fair usage.
...
Cached token information is only displayed when cached tokens are being used. This feature is available for API key users (LLM API key or Google Cloud Vertex AI) but not for OAuth users (such as Google Personal/Enterprise accounts like Google Gmail or Google Workspace, respectively). This is because the Gemini Code Assist API does not support cached content creation. You can still view your total token usage using the `/stats` command in TerminaI.

## Installation and updates

[Section titled “Installation and updates”](#installation-and-updates)

### How do I update TerminaI to the latest version?

[Section titled “How do I update TerminaI to the latest version?”](#how-do-i-update-terminai-to-the-latest-version)

If you installed it globally via `npm`, update it using the command `npm install -g @google/terminai@latest`. If you compiled it from source, pull the latest changes from the repository, and then rebuild using the command `npm run build`.
...
*   **Use a `.env` file:** Create a `.env` file in your project’s `.gemini` directory (`.gemini/.env`) and store your keys there. TerminaI will automatically load these variables.
...
### Where are the TerminaI configuration and settings files stored?

[Section titled “Where are the TerminaI configuration and settings files stored?”](#where-are-the-terminai-configuration-and-settings-files-stored)

The TerminaI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.terminai/settings.json`.
2.  In your project’s root directory: `./.gemini/settings.json`.

Refer to [TerminaI Configuration](/docs/get-started/configuration) for more details.
...
If you’re subscribed to Google AI Pro or Ultra, you automatically have higher limits to Gemini Code Assist and TerminaI. These are shared across TerminaI and agent mode in the IDE. You can confirm you have higher limits by checking if you are still subscribed to Google AI Pro or Ultra in your [subscription settings](https://one.google.com).

### What is the privacy policy for using Gemini Code Assist or TerminaI if I’ve subscribed to Google AI Pro or Ultra?

[Section titled “What is the privacy policy for using Gemini Code Assist or TerminaI if I’ve subscribed to Google AI Pro or Ultra?”](#what-is-the-privacy-policy-for-using-gemini-code-assist-or-terminai-if-ive-subscribed-to-google-ai-pro-or-ultra)
...
The higher limits in your Google AI Pro or Ultra subscription are for Gemini 2.5 across both Gemini 2.5 Pro and Flash. They are shared quota across TerminaI and agent mode in Gemini Code Assist IDE extensions. You can learn more about quota limits for TerminaI, Gemini Code Assist and agent mode in Gemini Code Assist at [Quotas and limits](https://developers.google.com/gemini-code-assist/resources/quotas).

### If I upgrade to higher limits for TerminaI and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?

[Section titled “If I upgrade to higher limits for TerminaI and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?”](#if-i-upgrade-to-higher-limits-for-terminai-and-gemini-code-assist-by-purchasing-a-google-ai-pro-or-ultra-subscription-will-gemini-start-using-my-data-to-improve-its-machine-learning-models)
...
Search the [TerminaI Q&A discussions on GitHub](https://github.com/google-gemini/terminai/discussions/categories/q-a) or [start a new discussion on GitHub](https://github.com/google-gemini/terminai/discussions/new?category=q-a)