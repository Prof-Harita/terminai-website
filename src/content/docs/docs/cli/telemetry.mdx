...
Learn how to enable and setup OpenTelemetry for TerminaI.
...
Built on **[OpenTelemetry](https://opentelemetry.io/)** — the vendor-neutral, industry-standard observability framework — TerminaI’s observability system provides:
...
Use CLI credentials for telemetry (GCP target only)
...
For detailed information about all configuration options, see the [Configuration guide](/docs/get-started/configuration).
...
[Section titled “Authenticating with CLI Credentials”](#authenticating-with-cli-credentials)

By default, the telemetry collector for Google Cloud uses Application Default Credentials (ADC). However, you can configure it to use the same OAuth credentials that you use to log in to the TerminaI. This is useful in environments where you don’t have ADC set up.

To enable this, set the `useCliAuth` property in your `telemetry` settings to `true`:

```
{  "telemetry": {    "enabled": true,    "target": "gcp",    "useCliAuth": true  }}
```
...
2.  Run TerminaI and send prompts.
...
3.  Run TerminaI and send prompts.
4.  View logs and metrics:
    *   Open the Google Cloud Console in your browser after sending prompts:
        *   Logs: [https://console.cloud.google.com/logs/](https://console.cloud.google.com/logs/)
        *   Metrics: [https://console.cloud.google.com/monitoring/metrics-explorer](https://console.cloud.google.com/monitoring/metrics-explorer)
        *   Traces: [https://console.cloud.google.com/traces/list](https://console.cloud.google.com/traces/list)
    *   Open `~/.terminai/tmp/<projectHash>/otel/collector-gcp.log` to view local collector logs.
...
The following section describes the structure of logs and metrics generated for TerminaI.
...
Logs are timestamped records of specific events. The following events are logged for TerminaI, grouped by category.
...
*   `gemini_cli.tool.call.count` (Counter, Int): Counts tool calls.
    ...
    *   **Attributes**:
        *   `function_name`
        *   `success` (boolean)
        *   `decision` (string: “accept”, “reject”, “modify”, or “auto\_accept”, if applicable)
...
*   `gemini_cli.api.request.breakdown` (Histogram, ms): API request time by phase.
    ...
    *   **Attributes**:
        *   `model` (string)
        *   `phase` (“request\_preparation”, “network\_latency”, “response\_processing”, “token\_processing”)
