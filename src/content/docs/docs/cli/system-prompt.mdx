...
The core system instructions that guide TerminaI can be completely replaced with your own Markdown file. This feature is controlled via the `GEMINI_SYSTEM_MD` environment variable.
...
You can set the environment variable temporarily in your shell, or persist it via a `.gemini/.env` file. See [Persisting Environment Variables](/docs/get-started/authentication#persisting-environment-variables).
