...
The Trusted Folders feature is a security setting that gives you control over which projects can use the full capabilities of the TerminaI. It prevents potentially malicious code from running by asking you to approve a folder before the CLI loads any project-specific configurations from it.
...
Once the feature is enabled, the first time you run the TerminaI from a folder, a dialog will automatically appear, prompting you to make a choice:
...
Your choice is saved in a central file (`~/.terminai/trustedFolders.json`), so you will only be asked once per folder.
...
When a folder is **untrusted**, the TerminaI runs in a restricted “safe mode” to protect you. In this mode, the following features are disabled:
...
6.  **MCP servers do not connect**: The CLI will not attempt to connect to any [Model Context Protocol (MCP)](/docs/tools/mcp-server) servers.
...
Granting trust to a folder unlocks the full functionality of the TerminaI for that workspace.
...
*   **View all trust rules**: To see a complete list of all your trusted and untrusted folder rules, you can inspect the contents of the `~/.terminai/trustedFolders.json` file in your home directory.
...
1.  **IDE trust signal**: If you are using the [IDE Integration](/docs/ide-integration), the CLI first asks the IDE if the workspace is trusted. The IDE’s response takes highest priority.
    
2.  **Local trust file**: If the IDE is not connected, the CLI checks the central `~/.terminai/trustedFolders.json` file.