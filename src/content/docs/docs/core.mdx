...
TerminaI’s core package (`packages/core`) is the backend portion of TerminaI, handling communication with the Gemini API, managing tools, and processing requests sent from `packages/cli`. For a general overview of TerminaI, see the [main documentation page](/docs).
