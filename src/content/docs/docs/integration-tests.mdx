...
The integration tests are designed to validate the end-to-end functionality of the TerminaI. They execute the built binary in a controlled environment and verify that it behaves as expected when interacting with the file system.
...
To regenerate these golden files, set the REGENERATE\_MODEL\_GOLDENS environment variable to “true” when running the tests, for example:
...
The workflow runs the tests in different sandboxing environments to ensure TerminaI is tested across each:
