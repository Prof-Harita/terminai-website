...
AI Studio user with a LLM API key
...
2.  **Use a `.env` file:** Create a `.gemini/.env` file in your project directory or home directory. TerminaI automatically loads variables from the first `.env` file it finds, searching up from the current directory, then in `~/.terminai/.env` or `~/.env`. `.gemini/.env` is recommended.
    ...
    ```
    mkdir -p ~/.geminicat >> ~/.terminai/.env <<'EOF'GOOGLE_CLOUD_PROJECT="your-project-id"# Add other variables like GEMINI_API_KEY as neededEOF
    ```
