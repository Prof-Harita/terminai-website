...
This document describes the `run_shell_command` tool for the TerminaI.
...
## How to use `run_shell_command` with the TerminaI

[Section titled “How to use run\_shell\_command with the TerminaI”](#how-to-use-run_shell_command-with-the-terminai)
...
You can configure the behavior of the `run_shell_command` tool by modifying your `settings.json` file or by using the `/settings` command in the TerminaI.
...
When an interactive command is running, you can send input to it from the TerminaI. To focus on the interactive shell, press `ctrl+f`. The terminal output, including complex TUIs, will be rendered correctly.
...
When `run_shell_command` executes a command, it sets the `GEMINI_CLI=1` environment variable in the subprocess’s environment. This allows scripts or tools to detect if they are being run from within the TerminaI.
